...
<div id="ek_waitList">
    ...
    <div class="tools form-inline">
    	<ul class="toolbar">
            ...
            &nbsp; &nbsp; &nbsp;
            <button class="btn btn-default" onclick="searchEk_wait()"><img src="../images/ico06.png" style="margin-top: -5px"/>搜索</button>
        </ul>
    ...
    <table class="table table-hover text-nowrap">
        ...
        <tr v-for="ek_wait in ek_wait_List">
            <td>{{ek_wait.wno}}</td>
            <td>{{ek_wait.pno}}</td>
            <td>{{ek_wait.name}}</td>
            <td>{{ek_wait.sex}}</td>
            <td>{{ek_wait.age}}</td>
            <td>{{ek_wait.type}}</td>
            <td>{{ek_wait.remark}}</td>
            <td>{{ek_wait.level}}</td>
            <td>{{ek_wait.date}}</td>
            <td>{{ek_wait.jzno}}</td>
            <td>{{ek_wait.dno}}</td>
            <td style="text-align: center">
                <button class="btn btn-default"><img src="../images/t02.png" v-on:click="updateEk_wait(ek_wait.id)">队列调整</button>
                <div class="btn-group">
                    <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        开具药单 <span class="caret"></span>
                    </button>
                    <ul class="dropdown-menu">
                        <li><a href="#" @click="addResult('开具开药单',ek_wait.pno)">开具开药单</a></li>
                        <li><a href="#" @click="addResult('开具皮试单',ek_wait.pno)">开具皮试单</a></li>
                        <li><a href="#" @click="addResult('开具注射单',ek_wait.pno)">开具注射单</a></li>
                        <li><a href="#" @click="addResult('开具输液单',ek_wait.pno)">开具输液单</a></li>
                    </ul>
                </div>
                <button class="btn btn-default"><img src="../images/t03.png" v-on:click="deleteById(deleteId=ek_wait.id)"> 完成就诊</button>
            </td>
            </tr>
    </table>
        <%-- 分页 begin--%>
        ...
        <!-- 模态弹出框 begin -->
        <div class="modal fade" id="ek_waitModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    ...
    <div class="tip">
      ...
      <div class="tipinfo">
        ...
        <div class="tipbtn">
        <input name="" type="button"  class="sure" value="确定" v-on:click="deleteEk_wait()"/>&nbsp;
...
<script src="../js/vue.js"></script>
     ...
     <script>
            var pageIndex = 1;
            var ek_waitVue = new Vue({
                el : "#ek_waitList",
                data : {
                    ek_wait_List : [],
                    ek_wait : {pno:"", name:"", height:"", weight:"", dno:"", jzno:""},//产科信息记录
...
//                    完成就诊
                    ...
                    deleteEk_wait: function(){
                        var _this = this;
                        $.ajax({
                            url: "/deleteEk_wait.action",
...
//                    隐藏模态框
                    hideModal : function(){
                        $("#ek_waitModal").modal("hide");
                    },
//                    显示模态框
                    showModal : function (){
                        $("#ek_waitModal").modal("show");
                    },
...
//            分页
            $(function(){
                ...
                $(".page").click(function(){
                    ...
                    var maxPage = ek_waitVue._data.maxPage;
                    if(selectPage == "首页"){
                        pageIndex = 1;
                    ...
                    $.ajax({
                        ...
                        type : "post",
                        success : function(data){
                            ek_waitVue._data.ek_wait_List = data.listData;
                            ek_waitVue._data.maxPage = data.maxPage;
                            ek_waitVue._data.currnetIndex = pageIndex;//设置当前页码为选中的页码
                        }
                    })
                })
